...
--dual은 연산,함수 결과 등을 확인할 때 사용하는 임시 테이블
SELECT 2+3 FROM dual;

-- 시퀀스의 다음 값으로 증가해라
SELECT test_seq1.nextval FROM dual;	--nextvalue
--맨 처음 nextval을 실행해야 currval(시퀀스의 현재값) 실행 가능
SELECT test_seq1.currval FROM dual;	--currentvalue
...
CREATE SEQUENCE test_seq2
	INCREMENT BY 2		--증감값 설정(음수도 설정 가능)
